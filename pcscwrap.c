...
char *sc_listreaders(void)
{
  ...
  {
     rc = SCardListReaders(hCtx, NULL, (char *) &c, &dw); 
     if (rc == SCARD_S_SUCCESS)
     {
        cc = (char *) malloc(dw);
        if (cc != NULL)
...
/* -------------------------------------------------------------------------- */
char * findfirststr(char *buf, char *tpl)
{
  char * rc=NULL;
  ...
  while (*(buf))
  {
    rc = (char *)x_strstr((const char *)buf, (const char *)tpl);
...
LONG sc_init (sc_context *ctx, char *rdr)
{
  ...
  if (rc==SCARD_S_SUCCESS)
  {
     rc=SCardListReaders(ctx->hCtx, NULL, (char *) &c, &dw); 
     if (rc==SCARD_S_SUCCESS)
     {
